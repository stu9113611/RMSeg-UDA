...
def main():
    ...
    logdir = "logs/rlmd/stadv4to1_rcs_b8_iter80000_subadv01"
    log_interval = 60
    val_interval = 2000
    ema_warmup = -1
    ...
    # rcs_cfg = None
    rcs_cfg = RCSConfig(
        file_path="data/rlmd/clear/rcs.json",
        # file_path="data/ceymo/clear/rcs.json",
    ...
    discriminators = [
        SegformerDiscriminator.from_pretrained(
            pretrained_model_name_or_path="nvidia/mit-b0",
            num_labels=1,
        ).to(device)
    ...
    if rcs_cfg is not None:
        source_train_dataset = RCSImgAnnDataset(
            # root="data/ceymo/clear/train",
            root="data/rlmd/clear/train",
        ...
        source_train_dataloader = source_train_dataset.get_loader(
        ...
        domain_class_weight = source_train_dataset.rcm.sampling_probs.to(device)
    else:
    ...
    target_train_dataloader = ImgAnnDataset(
        # root="data/ceymo/rainy/train",
        root="data/rlmd/night/train",
    ...
    source_val_dataloader = ImgAnnDataset(
        # root="data/ceymo/clear/val",
        root="data/rlmd/clear/val",
    ...
    target_val_dataloader = ImgAnnDataset(
        # root="data/ceymo/rainy/val",
        root="data/rlmd/night/val",
    ...
    dis_label = torch.zeros((batch_size, 1, *(crop_size))).to(device)
    weight = torch.zeros((batch_size, 1, *(crop_size))).to(device)
        ...
        for it in range(1, max_iters + 1):
            ...
            if it > ema_warmup and it % ema.update_every == 0:
                ...
                for i, discriminator in enumerate(discriminators):
                    ...
                    weight.fill_(0)
                    weight[(source_ann == i)[:, None, :]] = 1
                    dis_loss = F.binary_cross_entropy_with_logits(
                        source_dis_pred, dis_label, weight
                    ).mean()
                    (dis_loss * domain_loss_weight).backward(retain_graph=True)
                    source_dis_loss += dis_loss.item()
                ...
                for i, discriminator in enumerate(discriminators):
                    ...
                    weight.fill_(0)
                    weight[(target_ann.argmax(1) == i)[:, None, :]] = 1
                    dis_loss = F.binary_cross_entropy_with_logits(
                        target_dis_pred, dis_label, weight
                    ).mean()
                    (dis_loss * domain_loss_weight).backward(retain_graph=True)
                    target_dis_loss += dis_loss.item()
            ...
            if it % log_interval == 0:
                ...
                if it > ema_warmup and it % ema.update_every == 0:
                    ...
                    # )
                    logger.tb_log(
                        "Source Train Discriminator Loss", source_dis_loss, it
                    )
                    logger.tb_log(
                        "Target Train Discriminator Loss", target_dis_loss, it
                    )
                    # logger.tb_log(
